...
```{r}
#| better scatterplot with facets
ggplot(
       data = surveys, 
       mapping = aes(x = weight, 
                     y = hindfoot_length
       ...
       geom_point() + 
       facet_wrap(~species)
       #is there a way to change the scale of the graph so that each facet is wider?
```
...
ggplot(
       ...
       geom_point() + 
       facet_wrap(~species) + 
       labs(x = "weight (g)", y = "", title = "weight relation to hindfoot length in mm")
```
...
ggplot(
       ...
       geom_boxplot() + 
       labs(x = "weight (g)", y = "", title = "distribution of weight in grams by species")
```
...
#using help(geom_boxplot) to find how to modify outliers: help(geom_boxplot)
ggplot(
       data = surveys, 
       mapping = aes(x = weight, 
                     y = species
       ...
       geom_boxplot(outlier.alpha = 0) + #outlier.alpha found from help(geom_boxplot)
       geom_jitter(color = "blue", 
                   alpha = 0.1) +
       labs(x = "weight (g)", y = "", title = "distribution of weight in grams by species")
```
...
#source for coord_flip(): https://www.geeksforgeeks.org/how-to-create-horizontal-boxplots-in-r/
ggplot(
       data = surveys, 
              mapping = aes(x = weight, 
                            y = species
                            )) + 
       #outlier.alpha found from help(geom_boxplot)
...
```{r}
#| label: anova
#was used for documentation: help(aov)
species_mod <- aov(weight ~ species, surveys)
...
Conclusion:
with an F-value of 19916 and a p-value of $2 * 10 ^{-16}$, we reject the null hypothesis in favor of the alternate hypothesis stating there is at least one rodent species with a different mean weight.
